...
@Route(path = RouterTable.addCheck)
public class AddCheckActivity extends BaseActivity<InboundViewModel> {
    ...
    protected int getLayoutResId() {
        return R.layout.add_check_supplies;
    }
    ...
    public void add() {
        //TODO 验证数据完整
        if (check_name.getText().length() == 0) {
            showDialog("物资名称不能为空");
        } else if (specifications.getText().length() == 0) {
            showDialog("规格不能为空");
        } else if (date.getText().length() == 0) {
            showDialog("检定期不能为空");
        } else if (check_quantity.getText().length() == 0) {
            showDialog("数量不能为空");
