...
@Route(path = RouterTable.addOutbound)
public class AddOutboundActivity extends BaseActivity<InboundViewModel> {
    @BindView(R.id.outbound_name)
    EditText outbound_name;
    @BindView(R.id.specifications)
    EditText specifications;
    @BindView(R.id.date)
    EditText date;
    @BindView(R.id.outbound_quantity)
    EditText outbound_quantity;
    ...
    protected int getLayoutResId() {
        return R.layout.add_outbound_supplies;
    }
    ...
    public void add() {
        //TODO 验证数据完整
        if (outbound_name.getText().length() == 0) {
            showDialog("物资名称不能为空");
        } else if (specifications.getText().length() == 0) {
            showDialog("规格不能为空");
        } else if (date.getText().length() == 0) {
            showDialog("检定期不能为空");
        } else if (outbound_quantity.getText().length() == 0) {
            showDialog("数量不能为空");
